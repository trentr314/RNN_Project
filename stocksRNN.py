###############################################################
# Filename: stocksRNN.py
# Version: 1.0
# Author: Trent Rogers
# NetID: tar170230
# Email: tar170230@utdallas.edu
# Date: 2020-11-25
# Description:
...
import csv
# suppress the one warning I couldn't get around
import warnings
...
# the print switch was created to have the function print information only on certain batches or epochs
...
for epoch in range(n_epochs):
	...
	# all of the following ifs are technically unecessary, but are convenient when you are trying large epoch sizes
	# they become increasingly accurate as time elapsed increases
	if(epoch==9):
		epoch_time = datetime.datetime.now() - start_time
		projected_time = (epoch_time * n_epochs / 10) - epoch_time
	...
	if(epoch==49):
		epoch_time = datetime.datetime.now() - start_time
		projected_time = (epoch_time * n_epochs / 50) - epoch_time
		projected_time -= datetime.timedelta(microseconds=projected_time.microseconds)
		print('projected remaining runtime: ' + str(projected_time))
		sys.stdout.flush()
	if(epoch==99):
		epoch_time = datetime.datetime.now() - start_time
		projected_time = (epoch_time * n_epochs / 100) - epoch_time
		projected_time -= datetime.timedelta(microseconds=projected_time.microseconds)
		print('projected remaining runtime: ' + str(projected_time))
		sys.stdout.flush()
	if(epoch==199):
		epoch_time = datetime.datetime.now() - start_time
		projected_time = (epoch_time * n_epochs / 200) - epoch_time
		projected_time -= datetime.timedelta(microseconds=projected_time.microseconds)
		print('projected remaining runtime: ' + str(projected_time))
		sys.stdout.flush()
...
with open('stocksRNN_log.csv', 'a', newline='') as log:
	#headers = ['learning rate:', 'n_epochs:', 'hidden_size:', 'batch_size:', 'running time:', 'MSE:']
...
'''  The following code was used to obtain the data in the file IBM_full_daily_adjusted.csv  '''
'''
import requests
import csv
